...
def estimate_normals(
    ...
    query-local residual statistics. Thin sheets receive a third extrapolated
    correction; thicker, noise-prone patches borrow slight broad-scale support.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                normals = np.where(thin_sheet, extrapolated, broad_blend)
            else:
