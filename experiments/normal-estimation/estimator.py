...
THIRD_REFINEMENT_MIN_SUPPORT_FRACTION = 0.5
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet when its normalized weights retain enough effective samples.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                sufficient_support = (
                    effective_count
                    >= THIRD_REFINEMENT_MIN_SUPPORT_FRACTION * refinement_count
                )
                normals = np.where(
