...
THIRD_REFINEMENT_MIN_WEIGHT_MASS = 0.375
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                supported = retained_weight_mass >= THIRD_REFINEMENT_MIN_WEIGHT_MASS
                normals = np.where(thin_sheet & supported, refined_normals, normals)
            else:
