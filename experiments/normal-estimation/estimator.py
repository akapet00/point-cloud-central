...
THIRD_REFINEMENT_MAX_ASPECT_RATIO = 0.15
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    query-local residual statistics. A third step is accepted only when robust
    residual thickness is small relative to the patch's in-plane PCA spread.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        tangent_spread = np.empty_like(bandwidth)
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 1:
                tangent_spread = np.sqrt(
                    np.maximum(eigenvalues[:, 1:2] + eigenvalues[:, 2:3], scale_floor)
                )
                normals = refined_normals
            elif step == 2:
                thin_sheet = (
                    robust_scale <= THIRD_REFINEMENT_MAX_ASPECT_RATIO * tangent_spread
                )
