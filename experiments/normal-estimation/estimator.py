...
THIRD_NORMAL_EXTRAPOLATION = 0.2
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                aligned_normals = refined_normals * alignment[:, None]
                extrapolated_normals = aligned_normals + THIRD_NORMAL_EXTRAPOLATION * (
                    aligned_normals - normals
                )
                extrapolated_normals /= np.linalg.norm(
                    extrapolated_normals, axis=1, keepdims=True
                )
                normals = np.where(thin_sheet, extrapolated_normals, normals)
            else:
