...
THIRD_REFINEMENT_MIN_OFFSET = 0.25
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    materially offset local sheet, where another robust recentering is useful.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                offset_sheet = np.abs(residual_median) >= (
                    THIRD_REFINEMENT_MIN_OFFSET * robust_scale
                )
                normals = np.where(thin_sheet & offset_sheet, refined_normals, normals)
            else:
