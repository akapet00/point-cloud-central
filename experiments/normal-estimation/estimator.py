...
THIRD_TUKEY_EXPONENT = 63.0 / 32.0
...
def estimate_normals(
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            squared_residual = normalized * normalized
            inside = squared_residual < 1.0
            if step == 2:
                compact_profile = np.maximum(1.0 - squared_residual, 0.0)
                robust_weights = np.power(compact_profile, THIRD_TUKEY_EXPONENT)
            else:
                robust_weights = np.square(1.0 - squared_residual) * inside
