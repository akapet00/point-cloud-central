...
FOURTH_CONTINUATION_BLEND = 0.125
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet. One eighth of a fourth robust continuation is then blended
    into that accepted normal to damp its extra curvature-bias correction.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                normals = refined_normals * np.where(dot < 0.0, -1.0, 1.0)
                frontier_normals = normals + THIRD_NORMAL_EXTRAPOLATION * (
                    normals - two_step_normals
                )
                ...
                continued_normals *= np.where(dot < 0.0, -1.0, 1.0)
                blended_normals = (
