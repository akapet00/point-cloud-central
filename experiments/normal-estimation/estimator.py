...
THIRD_REFINEMENT_MIN_MASS = 0.46875
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet with enough retained kernel mass for a stable covariance.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                sufficient_mass = retained_mass >= THIRD_REFINEMENT_MIN_MASS
                normals = np.where(
                    thin_sheet & sufficient_mass, refined_normals, normals
                )
            else:
