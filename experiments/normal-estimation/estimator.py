...
THICK_SHEET_BROAD_NORMAL_BLEND = 0.003125
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet; other patches borrow a tiny amount of broad-scale stability.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                broad_dot = np.einsum(
                    "ni,ni->n", initial_normals, normals, optimize=True
                )[:, None]
                aligned_broad = initial_normals * np.where(broad_dot < 0.0, -1.0, 1.0)
                broad_blended = (
                    1.0 - THICK_SHEET_BROAD_NORMAL_BLEND
                ) * normals + THICK_SHEET_BROAD_NORMAL_BLEND * aligned_broad
                broad_blended /= np.linalg.norm(broad_blended, axis=1, keepdims=True)
                normals = np.where(thin_sheet, extrapolated, broad_blended)
            else:
