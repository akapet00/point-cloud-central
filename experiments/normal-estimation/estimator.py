...
def estimate_normals(
    ...
    query-local residual statistics. Two additional steps are used only for a
    thin local sheet, where continued redescending refinement targets coherent
    curvature and density bias rather than widespread positional noise.
    """
    del query_indices
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step >= 2:
                ...
                if step == 2:
                    ...
                    normals = np.where(thin_sheet, aligned_normals, normals)
                else:
