...
THICK_SHEET_BROAD_NORMAL_BLEND = 0.000390625
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                thick_normals = normals + THICK_SHEET_BROAD_NORMAL_BLEND * (
                    aligned_broad - normals
                )
                thick_normals /= np.linalg.norm(thick_normals, axis=1, keepdims=True)
                normals = np.where(thin_sheet, extrapolated, thick_normals)
            else:
