...
VERY_THICK_SHEET_MIN_THICKNESS = 0.2
VERY_THICK_SHEET_BROAD_NORMAL_BLEND = 0.05
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet; very thick patches instead borrow slight broad-scale stability.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                broad_blend = (
                    1.0 - VERY_THICK_SHEET_BROAD_NORMAL_BLEND
                ) * normals + VERY_THICK_SHEET_BROAD_NORMAL_BLEND * aligned_broad
                broad_blend /= np.linalg.norm(broad_blend, axis=1, keepdims=True)
                very_thick = robust_scale >= VERY_THICK_SHEET_MIN_THICKNESS * bandwidth
                normals = np.where(
                    thin_sheet,
                    extrapolated,
                    np.where(very_thick, broad_blend, normals),
                )
            else:
