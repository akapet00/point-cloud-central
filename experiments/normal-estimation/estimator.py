...
FOURTH_CONTINUATION_BLEND = 0.03125
MAD_TO_SIGMA = 1.4826
BATCH_SIZE = 2_048
...
def estimate_normals(
    ...
    local sheet. A strongly damped fourth continuation tests further convergence.
    """
    del query_indices
    ...
    for start in range(0, neighbor_indices.shape[0], BATCH_SIZE):
        ...
        for step, (tukey_cutoff, refinement_count, statistic_count) in enumerate(
            ...
            if step == 2:
                ...
                extrapolated *= np.where(dot < 0.0, -1.0, 1.0)
                continued = (
